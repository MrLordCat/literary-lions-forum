...
func GetAllPosts(db *sql.DB, postID int64, userID int64) ([]Post, error) {
	...
	query.WriteString(`
    SELECT p.id, p.title, p.content, u.username, u.id as author_id, p.created_at,
	...
	for rows.Next() {
		...
		err := rows.Scan(&p.ID, &p.Title, &p.Content, &authorName, &authorID, &p.CreatedAt, &p.Likes, &p.Dislikes)
		if err != nil {
...
func UpdatePost(db *sql.DB, postID int64, title, content string) error {
	// Подготавливаем SQL запрос на обновление поста
